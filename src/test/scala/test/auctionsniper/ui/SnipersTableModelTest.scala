...
class SnipersTableModelTest extends JMockSuite {
  ...
  private val sniper = new AuctionSniper(Item(ITEM_ID, 234), null)

  private var model = new SnipersTableModel

  private val withListener = FunFixture[TableModelListener](
    setup = _ => {
      val listener = context().mock(classOf[TableModelListener])
  ...
  private def assertRowMatchesSnapshot(row: Int, snapshot: SniperSnapshot): Unit = {
    ...
    assert(clue(cellValue(row, Column.LAST_PRICE)) == clue(snapshot.lastPrice))
    assert(clue(cellValue(row, Column.LAST_BID)) == clue(snapshot.lastBid))
